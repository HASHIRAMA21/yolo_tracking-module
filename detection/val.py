...
from trackers.utils import ROOT, WEIGHTS, DETECTION
...
class Evaluator:
    ...
    def eval(self, opt, seq_paths, save_dir, MOT_results_folder, val_tools_path, gt_folder, free_devices):
        ...
        if not self.opt.eval_existing:
            ...
            for i, seq_path in enumerate(seq_paths):
                ...
                p = subprocess.Popen(
                    args=[
                        sys.executable, str(DETECTION / "track.py"),
    ...
    def run(self, opt):
        ...
        """
        e = Evaluator(opt)
        val_tools_path = DETECTION / 'val_utils'
...
def parse_opt():
    ...
    parser.add_argument('--project', default=DETECTION / 'runs' / 'val', help='save results to project/name')
