...
#   docker build -t HASHIRAMA21/yolo_tracking-module .
...
RUN git clone --recurse-submodules https://github.com/HASHIRAMA21/yolo_tracking-module.git /usr/src/app
...
#
#       docker run -it --gpus all HASHIRAMA21/yolo_tracking-module bash
#
#   - run interactively with first and third GPU accessible:
#
#       docker run -it --gpus '"device=0, 2"' HASHIRAMA21/yolo_tracking-module bash
...
#
#       docker run -it --gpus all -d HASHIRAMA21/yolo_tracking-module
